...
dealAppend m (Combined k c) (WithIncrement inc) = Combined ((inv*k) `mod` m) ((inv*c) `mod` m)
...
withIncrement = WithIncrement <$> (string "deal with increment " >>= const (P.integerAnd' eof))
...
newIndex dSize i (WithIncrement inc) = (i * powMod dSize inc (totient dSize - 1)) `mod` dSize
newIndex dSize i (Cut c) = (i + c) `mod` dSize
newIndex dSize i (Combined k x) = (i*k + x) `mod` dSize

powerDeals dSize pow i (Combined k x) = (powMod dSize k pow * i + x * geoSum dSize k pow) `mod` dSize
