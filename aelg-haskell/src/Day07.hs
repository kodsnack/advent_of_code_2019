...
addInputsVM input vm = VM { _memory             = _memory vm
  ...
  run instr@(Instr (OpCode opCode opModes) args) vm =
   ...
   where
    ...
    outputFunction mem mode arg =
      \_ -> error
        ("Illegal mode " ++ show mode ++ " in output instruction paramter")
...
initVMs program = map (runVM . init)
  where init phase = initVM [phase] program
...
runPass vms i = foldl' runAmp (i, []) vms
  where
    runAmp (i, vms) vm@WaitingVM{} = (out, vms ++ [nextVM])
      where
        (out, nextVM) = getOutputVM . runVM $ addInputsVM [i] vm
