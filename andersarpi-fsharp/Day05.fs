...
module Part1 =
    ...
    let rec traceToCOM dist p =
        match Map.tryFind p orbits with
        | None -> dist
        | Some x -> traceToCOM (dist+1) x

    planets |> List.sumBy (traceToCOM 0) |> printfn "%O"

module Part2 =
    let rec traceToCOM dist p ps =
        match Map.tryFind p orbits with
        | None -> ps
        | Some p' -> traceToCOM (dist+1) p' ((p', dist+1)::ps)
