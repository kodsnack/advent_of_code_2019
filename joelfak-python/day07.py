...
def intcode(program, input=None):
    ...
    while oc.opcode != 99:
        if oc.opcode == 1: # Addition
            ...
            program[program[p+1]] = input.pop(0)
            p = p + 2
...
def runAmplifiers(program, phaseSettings):
    res = intcode(program.copy(), [phaseSettings[0], 0]).output[0]
    res = intcode(program.copy(), [phaseSettings[1], res]).output[0]
    res = intcode(program.copy(), [phaseSettings[2], res]).output[0]
    res = intcode(program.copy(), [phaseSettings[3], res]).output[0]
    res = intcode(program.copy(), [phaseSettings[4], res]).output[0]
    return res
